...
architecture NN22Control_main_behavior of NN22Control_main is
	...
	-- misc
	signal ResetTxFIFOxS : std_logic;
	...
	signal RxIndxS	: std_logic;
	signal TxIndxS	: std_logic;
...
begin
	...
	FpLEDxSO(0) <= RunxS;
	AccRunIndxSO <= RunxS;
	RPi0RunSigxSO <= IsRunningxS;
	FpLEDxSO(1) <= IsRunningxS;
	...
	-- component instances
	rx_led_ps : pulse_stretcher
	generic map (
		LENGTH => 2**20  -- 2^20/96e6 = 11 ms
	)
	port map (
		ClkxCI		=> ClkxCI,
		ResetxRI 	=> ResetxRI,
		PulseInxSI	=> RxIndxS,
		PulseOutxSO	=> FpLEDxSO(2)
	);
	
	tx_led_ps : pulse_stretcher
	generic map (
		LENGTH => 2**20  -- 2^20/96e6 = 11 ms
	)
	port map (
